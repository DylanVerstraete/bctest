...
daemonpkgs = ./cmd/bctempld
clientpkgs = ./cmd/bctemplc
...
configpkg = github.com/threefoldtech/rivine/pkg/config
ldflagsversion = -X $(configpkg).rawVersion=$(fullversion)

stdoutput = $(GOPATH)/bin
daemonbin = $(stdoutput)/bctempld
clientbin = $(stdoutput)/bctemplc
